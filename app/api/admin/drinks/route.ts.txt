...
/**
 * PATCH /api/admin/drinks
 * Body: { id: number, available: boolean }
 *
 * Toggle de disponibilidad de un trago. Disponible para ADMIN1 y ADMIN2.
 */
export async function PATCH(request: NextRequest) {
  ...
  const drink = await prisma.drink.update({ where: { id }, data: { available } });
